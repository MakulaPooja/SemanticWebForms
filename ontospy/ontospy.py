...
class Ontology(object):
	...
	def ontologyJsonObject(self, element = 0, treedict = None , level=0 ):
		global tid
		if not treedict:
			treedict = self.ontologyClassTree
		stringa = "\t"*level
		...
		for x in treedict[element]:
			...
			stringa += "{ id: " + str(tid)
			tid = tid +1
			stringa += ",text: \"%s\"" % uri2niceString(x, self.ontologyNamespaces).split(':')[1]
			stringa += self.ontologyJsonObject(x, treedict , level+1)
			stringa += "\t"*level
			stringa += "},\n"
		if(loop):
			stringa=stringa[:-2]
